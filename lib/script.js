...
// helpers
var javaHelper = null; // initialized later if needed
var fs = new DwarfFs();
...
function DwarfFs() {
    this.fclose = new NativeFunction(Module.findExportByName(libc, 'fclose'), 'int', ['pointer']);
    this.fcntl = new NativeFunction(Module.findExportByName(libc, 'fcntl'), 'int', ['int', 'int', 'int']);
    this.fgets = new NativeFunction(Module.findExportByName(libc, 'fgets'), 'int', ['pointer', 'int', 'pointer']);
    this.fileno = new NativeFunction(Module.findExportByName(libc, 'fileno'), 'int', ['pointer']);
    this.fputs = new NativeFunction(Module.findExportByName(libc, 'fputs'), 'int', ['pointer', 'pointer']);
    this.getline = new NativeFunction(Module.findExportByName(libc, 'getline'), 'int', ['pointer', 'pointer', 'pointer']);
    this.pclose = new NativeFunction(Module.findExportByName(libc, 'pclose'), 'int', ['pointer']);
    this._fopen = new NativeFunction(Module.findExportByName(libc, 'fopen'), 'pointer', ['pointer', 'pointer']);
    this._popen = new NativeFunction(Module.findExportByName(libc, 'popen'), 'pointer', ['pointer', 'pointer']);
...
function NativeHelper() {
    ...
    Interceptor.attach(Module.findExportByName(libc, 'clone'), {
        onEnter: function() {
            if (nativeHelper.follow) {
                var fn, child_stack, flags, args = null;
                if (Process.arch === 'arm') {
                    fn = this.context.r0;
                    child_stack = this.context.r1;
                    flags = this.context.r2;
                    args = this.context.r3;
                } else if (Process.arch === 'arm64') {
                    fn = this.context.x0;
                    child_stack = this.context.x1;
                    flags = this.context.x2;
                    args = this.context.x3;
                }
                send('helper_clone:::' + JSON.stringify({
                    'fn': fn,
    ...
    Interceptor.attach(Module.findExportByName(libc, 'fork'), {
        onEnter: function() {
            if (nativeHelper.follow) {
                send('helper_fork:::');
            }
    ...
    Interceptor.attach(Module.findExportByName(libc, 'vfork'), {
        onEnter: function() {
            if (nativeHelper.follow) {
                send('helper_vfork:::');
            }
...
var InterceptorWrapper = function() {
    ...
    this._replace = function(pt, replacement) {
        return wrappedInterceptor._replace(pt, replacement);
    };
    this.replace = function(pt, replacement) {
        return wrappedInterceptor.replace(pt, replacement);
    };
