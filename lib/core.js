...
function DwarfApi() {
    ...
    this.isprint_fn = null;
    var isprint_ptr = Module.findExportByName(null, 'isprint');
    if (isDefined(this.isprint_ptr)) {
        this.isprint_fn = new NativeFunction(isprint_ptr, 'int', ['int']);
    }
    ...
    this.enumerateStrings = function (start, size) {
        start = ptr(start);
        var _ret = [];
        var pos = 0;
        while(pos < size) {
            var increment = 1;
            try {
                var strval = Memory.readUtf8String(start.add(pos));
                if (strval.length > 1) {
                    if (!_ret.includes(strval)) {
                        _ret.push(strval);
                    }
                    increment = strval.length;
                }
            } catch (e) {
                // ignored
            }
    ...
    this.enumerateStringsModule = function(module) {
        module = Process.findModuleByName(module);
        var _ret = [];
        if (isDefined(module)) {
            _ret = this.enumerateStrings(module.base, module.size);
        }
        return _ret;
    };
    ...
    this.isPrintable = function (char) {
        try {
            if (isDefined(this.isprint_fn)) {
                return this.isprint_fn(char);
            } else {
                if ((char > 31) && (char < 127)) {
                    return true;
                }
